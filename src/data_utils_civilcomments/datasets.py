...
class BaseWildsDataset(SpuriousCorrelationDataset):
    def __init__(
        ...
        valid_indices = np.array([
            idx for idx in self.dataset.indices if (isinstance(self.dataset.dataset[idx][0], str) and self.dataset.dataset[idx][0].strip() != "")
        ])
        self.dataset.indices = valid_indices
        # -----------------------
